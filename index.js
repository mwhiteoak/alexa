...
var URL = "http://www.rivercitylabs.net/events/list/?ical";

// Skills name 
var skillName = "RCL calendar:";
        ...
        ical.fromURL(URL, {}, function (err, data) {
            ...
            for (var k in data) {
                if (data.hasOwnProperty(k)) {
                    ...
                    var eventData = {
                        summary: cleanOutput(ev.summary),
                        location: cleanOutput(ev.location),
                        description: cleanOutput(ev.description),
                        start: ev.start
                    }
                ...
                if (eventDate.startDate && eventDate.endDate) {
                    ...
                    if (relevantEvents.length > 0) {
                        ...
                        if (relevantEvents[0] != null) {
                            ...
                            output += utils.format(eventSummary, "First", cleanOutput(relevantEvents[0].summary), relevantEvents[0].location, date.toDateString() + ".");
                        }
                        if (relevantEvents[1]) {
                            var date = new Date(relevantEvents[1].start);
                            output += utils.format(eventSummary, "Second", cleanOutput(relevantEvents[1].summary), relevantEvents[1].location, date.toDateString() + ".");
                        }
                        if (relevantEvents[2]) {
                            var date = new Date(relevantEvents[2].start);
                            output += utils.format(eventSummary, "Third", cleanOutput(relevantEvents[2].summary), relevantEvents[2].location, date.toDateString() + ".");
                        }

                        for (var i = 0; i < relevantEvents.length; i++) {
                            var date = new Date(relevantEvents[i].start);
                            cardContent += utils.format(cardContentSummary, cleanOutput(relevantEvents[i].summary), cleanOutput(relevantEvents[i].location), date.toDateString()+ "\n\n");
                        }
...
var descriptionHandlers = Alexa.CreateStateHandler(states.DESCRIPTION, {
    'eventIntent': function () {
        ...
        if (relevantEvents[index]) {
            ...
            output = descriptionMessage + cleanOutput(relevantEvents[index].description);
...
function cleanOutput(str) {
    ...
    return str.replace('Level 2, 282 Wickham St , Fortitude Valley, QLD, 4006, Australia', "RCL");
}
